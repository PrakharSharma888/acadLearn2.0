...
const INPUT_CLS = "w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-orange-400";
...
const LockedField = ({ value }) => (
  <div className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-3 py-2">
    ...
    <span className="text-sm font-semibold text-orange-700 truncate">{value}</span>
  </div>
);
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {isLoggedIn ? <LockedField value={form.name} /> : (
                  <input name="name" value={form.name} onChange={handleChange} required placeholder="Student's full name" className={INPUT_CLS} />
                )}
              </div>
              ...
              <div>
                ...
                {isLoggedIn && form.batchYear ? <LockedField value={form.batchYear} /> : (
                  <input name="batchYear" value={form.batchYear} onChange={handleChange} placeholder="e.g. Grade 7 / Batch 2024–25" className={INPUT_CLS} />
                )}
              </div>
              ...
              <div>
                ...
                {isLoggedIn && form.college ? <LockedField value={form.college} /> : (
                  <input name="college" value={form.college} onChange={handleChange} placeholder="School / college name" className={INPUT_CLS} />
                )}
              </div>
              ...
              <div>
                ...
                {deptOptions.length > 0 ? (
                  ...
                  <input name="department" value={form.department} onChange={handleChange} placeholder="e.g. Science, Arts" className={INPUT_CLS} />
                )}
              </div>
            </div>
...
const JuniorEvents = () => {
  ...
  const loadEvents = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/events?page=junior`);
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF5] font-sans text-gray-800">
      ...
      {/* Hero strip */}
      <div className="bg-linear-to-r from-orange-500 to-amber-400 text-white py-12 px-4">
        <div className="container mx-auto max-w-5xl">
          <Link to="/junior" className="inline-flex items-center gap-1.5 text-orange-100 hover:text-white text-sm font-semibold mb-4 transition-colors">
          ...
          <p className="text-orange-100 text-lg">Workshops, hackathons &amp; live sessions — register and join the fun!</p>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-5xl px-4 py-8">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full border border-gray-200 rounded-2xl pl-10 pr-4 py-3 text-sm bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
            autoComplete="off"
          />
          {search && (
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400">
              {allEvents.length === 0
                ? "Events for Junior page will appear here once created by admin."
                : "Try clearing your search or changing the filter."}
            </p>
            {(search || pill !== "all") && (
              <button
                onClick={() => { setSearch(""); setPill("all"); }}
                className="mt-4 px-5 py-2 bg-orange-500 text-white text-sm font-semibold rounded-xl hover:bg-orange-600"
              >
                Show all events
              </button>
            )}
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-3xl overflow-hidden border flex flex-col transition-all ${
                    upcoming ? "border-orange-100 shadow-sm hover:shadow-md" : "border-gray-100 opacity-60"
                  }`}
                >
                  {/* Image */}
                  <div className="h-36 relative overflow-hidden">
                    {ev.imageUrl
                      ...
                      : <div className={`w-full h-full ${upcoming ? "bg-linear-to-br from-orange-400 to-amber-300" : "bg-linear-to-br from-gray-300 to-gray-200"}`} />
                    }
                    {/* Badges */}
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <p className={`text-xs font-semibold mb-1 ${upcoming ? "text-orange-500" : "text-gray-400"}`}>
